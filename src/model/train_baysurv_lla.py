...
if __name__ == "__main__":
    ...
    for dataset_name in DATASETS:
        ...
        for model_name, trainer in zip(model_names, trainers):
            ...
            path = Path.joinpath(pt.MODELS_DIR, f"{dataset_name.lower()}_{model_name.lower()}_{loss_fn_name.lower()}/")
            model.save_weights(path)

        # Save results
        print(results)
        results.to_csv(Path.joinpath(pt.RESULTS_DIR, f"baysurv_{dataset_name.lower()}_{loss_fn_name.lower()}_results.csv"), index=False)