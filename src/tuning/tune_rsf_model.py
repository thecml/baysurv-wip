...
from utility.tuning import get_rsf_sweep_config
import argparse
...
def train_model():
    ...
    for train, test in kf.split(T1[0], T1[1]):
        ...
        test_surv_fn = pd.DataFrame(model.predict_survival(cvi_X, times=list(times)), columns=times)
